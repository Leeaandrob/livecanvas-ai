 ...
 * Analyze Route
 *
 * POST /api/analyze - Analyze a Mermaid diagram and explain its architecture
 */
...
import { ANALYZE_PROMPT, SYSTEM_PROMPT, buildPrompt } from "../lib/prompts";
...
analyzeRoute.post("/", async (c) => {
  try {
    ...
    // Build the prompt
    const prompt = buildPrompt(ANALYZE_PROMPT, { MERMAID: mermaid });
    ...
    c.executionCtx.waitUntil(
      (async () => {
        // Wait for stream to finish (simplified - in production use a proper mechanism)
